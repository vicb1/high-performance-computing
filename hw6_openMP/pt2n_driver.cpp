...
int main(int argc, char *argv[]) {
  ...
  // std::cout << "" << "Sequential twoNorm answer: " << seqAns << "time: " << seqElapsed.count() << std::endl;
  std::cout << vectorSize << " " << seqElapsed.count()/numRep << " " << parElapsed.count()/numRep << " " << (seqElapsed.count() / parElapsed.count()) << " "<< (seqAns - parAns) << std::endl;
