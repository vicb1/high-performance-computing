...
void task_matvec(const Matrix& A, const Vector& x, Vector& y, size_t partitions) {
  ...
  for(unsigned long k = 0; k<partitions; ++k){
    threads.push_back(std::thread(matvec_helper, std::cref(A), std::cref(x), std::ref(y), std::cref(rowsNum)*k/std::cref(partitions), std::cref(rowsNum)*(k+1)/std::cref(partitions)));
  }
  for(unsigned long k = 0; k<partitions; ++k){
    threads[k].join();
  }
