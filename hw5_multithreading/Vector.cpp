...
double partitionedTwoNorm(const Vector& x, size_t partitions) {
  ...
  for(unsigned long k = 0; k<partitions; ++k){
    threads.push_back(std::thread(ptn_worker, std::cref(x), std::cref(rowsNum)*k/std::cref(partitions), std::cref(rowsNum)*(k+1)/std::cref(partitions), std::ref(partial)));
    }
    for(unsigned long k = 0; k<partitions; ++k){
      threads[k].join();
    }
...
double recursiveTwoNorm(const Vector& x, size_t levels){
  ...
  std::mutex normMutex;
  normMutex.lock();
  std::future<double> parAns2 = std::async(std::launch::deferred, rtn_worker, std::cref(x), 0, std::cref(rowsNum), levels);
  normMutex.unlock();
