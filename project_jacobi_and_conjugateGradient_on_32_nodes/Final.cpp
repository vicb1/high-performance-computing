...
size_t ir(const mpiStencil& A, Grid& x, const Grid& b, size_t max_iter, double tol) {
  ...
  for (size_t iter = 0; iter < max_iter; ++iter) {
    ...
    MPI::COMM_WORLD.Sendrecv(&sendBottom[0], x.numX(), MPI::DOUBLE, next, 42, &recvTop[0], x.numX(), MPI::DOUBLE, prev, 42);
    for(int i = 0; i<x.numX(); ++i)
      x(i, 0) = recvTop[i];

    for(int i = 0; i<x.numX(); ++i)
      sendTop[i]=x(i,1);
    MPI::COMM_WORLD.Sendrecv(&sendTop[0], x.numX(), MPI::DOUBLE, prev, 42, &recvBottom[0], x.numX(), MPI::DOUBLE, next, 42);
    for(int i = 0; i<x.numX(); ++i)
...
size_t cg(const mpiStencil& A, Grid& x, const Grid& b, size_t max_iter, double tol){
  ...
  for (size_t iter = 0; iter < max_iter; ++iter) {
    ...
    MPI::COMM_WORLD.Sendrecv(&sendBottom[0], p.numY(), MPI::DOUBLE, next, 42, &recvTop[0], p.numY(), MPI::DOUBLE, prev, 42);
    for(int i = 0; i<p.numY(); ++i)
      p(0, i) = recvTop[i];

    for(int i = 0; i<p.numY(); ++i)
      sendTop[i]=p(1, i);
    MPI::COMM_WORLD.Sendrecv(&sendTop[0], p.numY(), MPI::DOUBLE, prev, 42, &recvBottom[0], p.numY(), MPI::DOUBLE, next, 42);
    for(int i = 0; i<p.numY(); ++i)
